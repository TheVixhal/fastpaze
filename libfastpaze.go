...
func ServeOpenAPI(ctx *fasthttp.RequestCtx) {
	openapi := OpenAPI{
		...
		Info: map[string]string{
			"title":       "FastPaze API",
			"version":     "1.0.0",
			"description": "High-performance API built with FastPaze and Go",
		},
...
func StartServer() {
	...
	go func() {
		logInfo("FastPaze server running on http://localhost:8080")
		logInfo("API docs available at http://localhost:8080/swagger/")
...
func SetServerConfig(cReadTimeout uintptr, cWriteTimeout uintptr, cIdleTimeout uintptr, cMaxBodySize uintptr, cConcurrency uintptr) {
	...
	logInfo("Server config updated: ReadTimeout=%v, WriteTimeout=%v, IdleTimeout=%v, MaxBodySize=%d, Concurrency=%d",
		serverConfig.ReadTimeout, serverConfig.WriteTimeout, serverConfig.IdleTimeout,
		serverConfig.MaxRequestBodySize, serverConfig.Concurrency)
}
